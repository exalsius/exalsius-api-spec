...
[**generate_self_register_token**](ManagementApi.md#generate_self_register_token) | **POST** /management/self-register-token | Generate a self-register token
[**get_dashboard_url**](ManagementApi.md#get_dashboard_url) | **GET** /management/dashboard-url | Get dashboard URL
[**get_self_register_token**](ManagementApi.md#get_self_register_token) | **GET** /management/self-register-token | Get the active self-register token
...
# **generate_self_register_token**
> SelfRegisterTokenResponse generate_self_register_token()

Generate a self-register token

**Generate a self-register token**

Generate a self-register token that can be used by a node agent to bootstrap itself through
`POST /node/self-register`.

If a self-register token already exists for your account or tenant bootstrap flow, this endpoint rotates it
and returns the newly active token.

**Result**

Returns the active self-register token.
...
with exalsius_api_client.ApiClient(configuration) as api_client:
    ...
    try:
        # Generate a self-register token
        api_response = api_instance.generate_self_register_token()
        print("The response of ManagementApi->generate_self_register_token:\n")
        pprint(api_response)
    except Exception as e:
        print("Exception when calling ManagementApi->generate_self_register_token: %s\n" % e)
```
...
[**SelfRegisterTokenResponse**](SelfRegisterTokenResponse.md)
...
| Status code | Description | Response headers |
|-------------|-------------|------------------|
**200** | Self-register token response |  -  |
**400** | Error response |  -  |
**500** | Error response |  -  |
...
**Retrieve the URL to the tenant-scoped Grafana instance with various dashboards**

Returns the URL to the tenant-scoped Grafana instance with various dashboards.
...
# **get_self_register_token**
> SelfRegisterTokenResponse get_self_register_token()

Get the active self-register token

**Get the active self-register token**

Retrieve the currently active self-register token that can be used by a node agent to bootstrap itself through
`POST /node/self-register`.
...
with exalsius_api_client.ApiClient(configuration) as api_client:
    ...
    try:
        # Get the active self-register token
        api_response = api_instance.get_self_register_token()
        print("The response of ManagementApi->get_self_register_token:\n")
        pprint(api_response)
    except Exception as e:
        print("Exception when calling ManagementApi->get_self_register_token: %s\n" % e)
```

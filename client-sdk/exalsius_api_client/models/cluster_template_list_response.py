...
class ClusterTemplateListResponse(BaseModel):
    """
    ClusterTemplateListResponse
    """  # noqa: E501

    cluster_templates: List[ClusterTemplate]
    total: Optional[StrictInt] = Field(
        default=None, description="The total number of available cluster templates"
    )
    __properties: ClassVar[List[str]] = ["cluster_templates", "total"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in cluster_templates (list)
        _items = []
        if self.cluster_templates:
            for _item_cluster_templates in self.cluster_templates:
                if _item_cluster_templates:
                    _items.append(_item_cluster_templates.to_dict())
            _dict["cluster_templates"] = _items
        return _dict
        ...
        _obj = cls.model_validate(
            {
                "cluster_templates": (
                    [
                        ClusterTemplate.from_dict(_item)
                        for _item in obj["cluster_templates"]
                    ]
                    if obj.get("cluster_templates") is not None
                    else None
                ),
                "total": obj.get("total"),
            }

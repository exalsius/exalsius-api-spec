...
class ClusterLocalStorage(BaseModel):
    """
    Local storage configuration
    """  # noqa: E501

    enabled: Optional[StrictBool] = Field(
        default=True, description="Enable local storage provisioner"
    )
    base_path: Optional[StrictStr] = Field(
        default="/var/openebs/local",
        description="Base path for local storage. If empty, /var/openebs/local will be used",
        alias="basePath",
    )
    __properties: ClassVar[List[str]] = ["enabled", "basePath"]
        ...
        _obj = cls.model_validate(
            {
                ...
                "basePath": (
                    obj.get("basePath")
                    if obj.get("basePath") is not None
                    else "/var/openebs/local"
                ),
            }
        )
        return _obj

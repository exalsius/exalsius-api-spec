...
class WorkspaceTemplateListResponse(BaseModel):
    ...
    total: Optional[StrictInt] = Field(
        default=None, description="The total number of workspace templates"
    )
    __properties: ClassVar[List[str]] = ["workspace_templates", "total"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in workspace_templates (list)
        _items = []
        if self.workspace_templates:
            for _item_workspace_templates in self.workspace_templates:
                if _item_workspace_templates:
                    _items.append(_item_workspace_templates.to_dict())
            _dict["workspace_templates"] = _items
        return _dict
        ...
        _obj = cls.model_validate(
            {
                "workspace_templates": (
                    [
                        WorkspaceTemplate.from_dict(_item)
                        for _item in obj["workspace_templates"]
                    ]
                    if obj.get("workspace_templates") is not None
                    else None
                ),
                "total": obj.get("total"),
            }

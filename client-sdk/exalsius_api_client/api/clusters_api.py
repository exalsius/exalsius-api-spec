...
class ClustersApi:
    ...
    def create_cluster(
        ...
        **Create a cluster**  Create a new cluster.  **Parameters**  - `name`: The name of the cluster (required) - `cluster_type`: The type of cluster (required). Possible values: `CLOUD`, `REMOTE`, `ADOPTED`, `DOCKER` - `vpn_cluster`: Whether the cluster should be provisioned with VPN connectivity (optional, defaults to `false`) - `telemetry_enabled`: Enables observability/monitoring integrations for the cluster (optional, defaults to `false`) - `colony_id`: The ID of the colony to add the cluster to (optional) - `cluster_labels`: Arbitrary key/value labels applied to the cluster (optional) - `cluster_annotations`: Arbitrary key/value annotations applied to the cluster (optional) - `machine_pre_start_commands`: Commands to run on each machine before the cluster starts (optional) - `local_storage`: Local storage provisioner config. Supports `enabled` (bool, default `true`) and `basePath` (string) (optional) - `k8s_version`: The Kubernetes version of the cluster (optional, defaults based on cluster type) - `to_be_deleted_at`: The date and time the cluster will be deleted (optional) - `control_plane_node_ids`: The IDs of the control plane nodes (optional) - `worker_node_ids`: The IDs of the worker nodes (optional) - `service_deployments`: The services to deploy in the cluster (optional)  If `to_be_deleted_at` is provided, the cluster will automatically be deleted at the specified date and time. If `control_plane_node_ids` or `worker_node_ids` are provided, the nodes will be added to the cluster. If `service_deployments` are provided, the services will be deployed in the cluster. If `local_storage` is configured, the storage provisioner will adopt the supplied settings when the cluster becomes ready.  **Behavior:** - A new cluster resource will be created with the specified configuration - The cluster will be in the `PENDING` state until deployment is initiated - You must call `POST /cluster/{cluster_id}/deploy` to actually deploy the cluster - If nodes are specified, they will be associated with the cluster but not deployed until the deploy operation is called - If `service_deployments` are specified, services will be deployed after the cluster is ready
    ...
    def get_cluster_logs(
        self,
        cluster_id: Annotated[
            StrictStr, Field(description="The ID of the cluster to get the logs for")
        ],
        _request_timeout: Union[
            None,
            Annotated[StrictFloat, Field(gt=0)],
        ...
        **Retrieve the cluster logs**  Retrieve the logs for a specific cluster. The logs are returned in real-time as they are generated. This endpoint returns a NDJSON stream, so individual log objects are separated by a newline character.
        ...
        _param = self._get_cluster_logs_serialize(
            cluster_id=cluster_id,
            _request_auth=_request_auth,
            _content_type=_content_type,
            _headers=_headers,
    ...
    @validate_call
    def get_cluster_logs_with_http_info(
        self,
        cluster_id: Annotated[
            StrictStr, Field(description="The ID of the cluster to get the logs for")
        ],
    ...
    @validate_call
    def get_cluster_logs_without_preload_content(
        self,
        cluster_id: Annotated[
            StrictStr, Field(description="The ID of the cluster to get the logs for")
        ],
    ...
    def _get_cluster_logs_serialize(
        self,
        cluster_id,
        _request_auth,
        _content_type,
        ...
        return self.api_client.param_serialize(
            method="GET",
            resource_path="/cluster/{cluster_id}/logs",
            path_params=_path_params,
            query_params=_query_params,
            header_params=_header_params,
            body=_body_params,

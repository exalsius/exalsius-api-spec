...
class NodesApi:
    ...
    def self_register_node(
        self,
        node_self_register_request: NodeSelfRegisterRequest,
        _request_timeout: Union[
            None,
            Annotated[StrictFloat, Field(gt=0)],
            Tuple[
        ...
        **Self-register a node**  Use this endpoint as the bootstrap entrypoint for a node agent that needs to register itself with exalsius. The agent authenticates with a one-time or pre-issued `register_token` and submits the SSH and node metadata required to create or attach the node.  **Parameters**  To self-register a node, provide: - The `register_token` issued for the node bootstrap flow - The node hostname - The SSH endpoint reachable by exalsius (for example `192.168.1.1:22`) - The SSH username - The SSH key ID that should be used for ongoing management  Optional hardware, software, and system details can be included during registration.  **Result**  Returns the created node ID and the next access token the node agent should use for subsequent authenticated operations.

        :param node_self_register_request: (required)
        :type node_self_register_request: NodeSelfRegisterRequest
        :param _request_timeout: timeout setting for this request. If one
                                 number provided, it will be total request
                                 timeout. It can also be a pair (tuple) of
                                 (connection, read) timeouts.
        ...
        _param = self._self_register_node_serialize(
            node_self_register_request=node_self_register_request,
            _request_auth=_request_auth,
            _content_type=_content_type,
            _headers=_headers,
            _host_index=_host_index,
    ...
    @validate_call
    def self_register_node_with_http_info(
        self,
        node_self_register_request: NodeSelfRegisterRequest,
        _request_timeout: Union[
            None,
        ...
        _headers: Optional[Dict[StrictStr, Any]] = None,
        _host_index: Annotated[StrictInt, Field(ge=0, le=0)] = 0,
    ) -> ApiResponse[NodeSelfRegisterResponse]:
        """Self-register a node using a registration token
    ...
    @validate_call
    def self_register_node_without_preload_content(
        self,
        node_self_register_request: NodeSelfRegisterRequest,
        _request_timeout: Union[
            None,
    ...
    def _self_register_node_serialize(
        self,
        node_self_register_request,
        _request_auth,
        _content_type,
        _headers,
        _host_index,
        ...
        # process the form parameters
        # process the body parameter
        if node_self_register_request is not None:
            _body_params = node_self_register_request
        ...
        return self.api_client.param_serialize(
            method="POST",
            resource_path="/node/self-register",
            path_params=_path_params,
            query_params=_query_params,
            header_params=_header_params,
            body=_body_params,
